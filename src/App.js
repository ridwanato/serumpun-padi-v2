...
function App() {
  ...
  const {
    layers, sawahStatus, setSawahStatus,
    loading: kmzLoading, error: kmzError,
  ...
  const filteredKec = layers.kecamatan.filter(f =>
    activeKecNames.includes(f.properties?.name || '')
  );
  ...
  const activeSawah = activeSawahId
    ? layers.sawah.find(f => f._id === activeSawahId)
    : null;
  ...
  const toggleAllKec = () => {
    const next = {}; ALL_KEC.forEach(n => next[n] = !allKecChecked); setSelectedKec(next);
  };
  const toggleAllKel = () => {
    const next = {}; visibleKelList.forEach(n => next[n] = !allKelChecked); setSelectedKel(next);
  };
  ...
  const getSawahStyle = () => ({
    color: '#4ade80', weight: 2, fillOpacity, fillColor: '#cccccc',
  });

  const onEachSawah = (feature, layer) => {
    layer.on('click', () => {
      setActiveSawahId(feature._id);
      openPanel('sawah_detail');
    });
    layer.bindPopup(`<b>${feature.properties?.name || 'Sawah'}</b>`);
  };
  ...
  const handleHamburger = () => {
    isPanelOpen ? closePanel() : openPanel('dashboard');
  };
  ...
  const zoomToSawah = (feature) => {
    ...
    setActiveSawahId(feature._id);
    closePanel();
    mapRef.current.flyToBounds([[bbox[1], bbox[0]], [bbox[3], bbox[2]]], {
  ...
  const renderPanelContent = () => {
    switch (panelView) {
      ...
      case 'sawah_detail':
        return (
          <SawahDetail
            activeSawah={activeSawah} sawahStatus={sawahStatus}
            fillOpacity={fillOpacity} onFillOpacityChange={setFillOpacity}
      ...
      case 'rekap_luas':
        ...
        case 'rekap_produksi':
        return (
          <RekapProduksi filteredSawah={filteredSawah} sawahStatus={sawahStatus} />
        );
        default:
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: '#f0fdf4' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          {kmzError && <p style={{ color: 'red', fontSize: 12 }}>{kmzError}</p>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="sp-app" data-drawmode={drawMode || ''}>
      <div className={`sp-header${isPanelOpen ? ' sp-header--hidden-mobile' : ''}`}>
      ...
      <div className={`sp-panel${isPanelOpen ? ' is-open' : ''}`}>
        <PanelHeader panelView={panelView} onClose={closePanel} onBack={goBack}
          user={user} onLogin={() => setShowAuth(true)} onLogout={() => setUser(null)} />
        <div className="sp-panel__body">{renderPanelContent()}</div>
      </div>

      {showDrawBar && (
        <DrawToolbar drawMode={drawMode} triggerDraw={triggerDraw} triggerEdit={triggerEdit}
          triggerDelete={triggerDelete} finishDrawMode={finishDrawMode} cancelDrawMode={cancelDrawMode} />
      )}
